...
class StatsView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        var startAngl = -90F
        date.forEachIndexed { index, datum ->
            val angle = datum / date.sum() * 360F
            paint.color = colors.getOrElse(index) { generateRandomColor() }
            canvas.drawArc(oval, startAngl, angle, false, paint)
            startAngl += angle
        }

        paint.color = colors.getOrElse(0) { generateRandomColor() }
        canvas.drawArc(oval, startAngl, 10F, false, paint)
